...
export default function InfoContainer({enemyPokemon, allyPokemon, setGameData, handleAttack}) {
    ...
    return (
        <div className='h-[10%] text-sm text-center flex flex-col content-evenly items-center' data-testid='info-container'>
            {!battlehelper.gameData.gameOver ? (
                <div>
                    <p className='info'>{battlehelper.gameData.info}</p>
                    <button className='absolute right-[30%] bottom-[5%] px-2 py-1 border border-solid border-[#fcfaf8] rounded-[5px] bg-[#fcfaf8] text-[#031e19] cursor-pointer transition-all duration-100 ease-in-out text-[0.875rem] hover:bg-[#cb2327] hover:border-[#cb2327] hover:border-solid hover:text-[#fcfaf8]' onClick={() => handleAttack()}>Attack
                    </button>
                </div>
            ) : (
                <div>
